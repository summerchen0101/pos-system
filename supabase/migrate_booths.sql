-- Multi-booth: booths table, booth_id on orders & promotions, checkout RPC.
...
-- Stable default booth for backfill (existing rows).
insert into public.booths (id, name, location)
values ('00000000-0000-0000-0000-000000000001', '預設攤位', null)
on conflict (id) do nothing;

alter table public.orders add column if not exists booth_id uuid references public.booths (id);

update public.orders
set booth_id = '00000000-0000-0000-0000-000000000001'
where booth_id is null;

alter table public.orders alter column booth_id set not null;
alter table public.orders alter column booth_id set default '00000000-0000-0000-0000-000000000001';

create index if not exists orders_booth_id_idx on public.orders (booth_id);

alter table public.promotions add column if not exists booth_id uuid references public.booths (id);

update public.promotions
set booth_id = '00000000-0000-0000-0000-000000000001'
where booth_id is null;

alter table public.promotions alter column booth_id set not null;
alter table public.promotions alter column booth_id set default '00000000-0000-0000-0000-000000000001';
...
grant execute on function public.checkout_order_deduct_stock(integer, integer, integer, jsonb, jsonb, uuid) to anon;
