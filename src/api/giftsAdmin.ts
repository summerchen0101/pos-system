...
export async function listGiftsAdmin(): Promise<AdminGift[]> {
  const { data, error } = await supabase.from('gifts').select(giftSelect).order('name')
...
export async function getGiftById(id: string): Promise<AdminGift> {
  const { data, error } = await supabase.from('gifts').select(giftSelect).eq('id', id).single()
  if (error) throw error
...
export async function setGiftStock(giftId: string, stock: number): Promise<void> {
